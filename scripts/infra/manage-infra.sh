...
main() {
    ...
    if ! exec_sudo test -f "${MARKER_FILE}"; then
        ...
        # Create the file without sudo first so environment variables are properly expanded
...
EOF
        ...
        # Copy the file to /tmp/env_vars with proper permissions
        exec_sudo cp "${GLOBAL_ENV_VARS_FILE}" /tmp/env_vars
        exec_sudo chmod 644 /tmp/env_vars

        # Also ensure the original file has proper permissions for cleanup
        chmod 644 "${GLOBAL_ENV_VARS_FILE}"

        # Debug: Verify the environment variables were written correctly
        echo "=== Debug: Contents of environment variables file ==="
        cat /tmp/env_vars
        echo "=== End of environment variables file ==="
